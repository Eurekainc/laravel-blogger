...
use HessamDev\Hessam\Interfaces\SearchResultInterface;
use HessamDev\Hessam\Scopes\HessamPublishedScope;

/**
 * Class HessamPost
 * @package HessamDev\Hessam\Models
 */
class HessamPost extends Model implements SearchResultInterface
{
    ...
    protected static function boot()
    {
        ...
        static::addGlobalScope(new HessamPublishedScope());
    }
    ...
    public function url()
    {
        return route("hessam.single", $this->slug);
    }
    ...
    {
        return route("hessam.admin.edit_post", $this->id);
    }
    ...
    public function image_url($size = 'medium')
    {
        ...
        return asset(config("hessam.blog_upload_dir", "blog_images") . "/" . $filename);
    }
    ...
    {
        if (config("hessam.use_custom_view_files") && $this->use_view_file) {
            // using custom view files is enabled, and this post has a use_view_file set, so render it:
            $return = view("hessam::partials.use_view_file", ['post' => $this])->render();
        } else {
        ...
        if (!config("hessam.echo_html")) {
            // if this is not true, then we should escape the output
            if (config("hessam.strip_html")) {
                $return = strip_tags($return);
            }

            $return = e($return);
            if (config("hessam.auto_nl2br")) {
                $return = nl2br($return);
            }
        }
    ...
    protected function check_valid_image_size(string $size = 'medium')
    {
        ...
        if (array_key_exists("image_" . $size, config("hessam.image_sizes"))) {
            return true;
        }
        ...
        if (starts_with($size, "image_")) {
            ...
            /* the config/hessam.php and the DB columns SHOULD have keys that start with image_$size
            however when using methods such as image_url() or has_image() it SHOULD NOT start with 'image_'

            To put another way: :
                in the config/hessam.php : config("hessam.image_sizes.image_medium")
                in the database table:    : hessam_posts.image_medium
                when calling image_url()  : image_url("medium")
            */
            throw new \InvalidArgumentException("Invalid image size ($size). HessamPost image size should not begin with 'image_'. Remove this from the start of $size. It *should* be in the hessam.image_sizes config though!");
        }


        throw new \InvalidArgumentException("HessamPost image size should be 'large','medium','thumbnail' or another field as defined in config/hessam.php. Provided size ($size) is not valid");
    }
