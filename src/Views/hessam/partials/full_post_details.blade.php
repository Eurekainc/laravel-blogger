...
<p class="blog_body_content">
    ...
    {{--@if(config("hessam.use_custom_view_files")  && $post->use_view_file)--}}
    {{--                                // use a custom blade file for the output of those blog post--}}
    {{--   @include("hessam::partials.use_view_file")--}}
    {{--@else--}}
...
@includeWhen($post->author,"hessam::partials.author",['post'=>$post])
@includeWhen($post->categories,"hessam::partials.categories",['post'=>$post])
