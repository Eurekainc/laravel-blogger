...
@else
    ...
    @if(\Auth::check() && \Auth::user()->canManageHessamPosts())
         {{--is logged in + canManageHessamPosts() == true, so show a detailed error--}}
        <div class='alert alert-danger'>Custom blog post blade view file
                        (<code>{{$post->full_view_file_path()}}</code>) not found. <a
                    href='https://hessam.binshops.com/laravel/packages/help-documentation/laravel-blog-package-hessam'
                    target='_blank'>See Laravel Blog Package help here</a>.
                    </div>

    @else
        {{--is not logged in, or User::canManageHessamPosts() for current user == false--}}
