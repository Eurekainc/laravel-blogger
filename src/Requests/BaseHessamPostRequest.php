...
abstract class BaseHessamPostRequest extends BaseRequest
{
    ...
    protected function baseBlogPostRules()
    {
        ...
        if (config('hessam.use_custom_view_files')) {
        ...
        foreach ((array)config('hessam.image_sizes') as $size => $image_detail) {
            if ($image_detail['enabled'] && config("hessam.image_upload_enabled")) {
