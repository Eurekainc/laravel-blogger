...
class AddNewCommentRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        if (\Auth::check() && config("hessam.comments.save_user_id_if_logged_in", true)) {
        ...
        if (config("hessam.captcha.captcha_enabled")) {
            ...
            /** @var \HessamDev\Hessam\Interfaces\CaptchaInterface $captcha */
        ...
        if (config("hessam.comments.rules") && is_callable(config("hessam.comments.rules"))) {
            /** @var callable $func */
            $func = config('hessam.comments.rules');
            $return = $func($return);
        }

        if (config("hessam.comments.require_author_email")) {
