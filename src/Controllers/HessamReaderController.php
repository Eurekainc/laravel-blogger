...
use HessamDev\Hessam\Captcha\UsesCaptcha;
...
class HessamReaderController extends Controller
{
    ...
    {
        // the published_at + is_published are handled by HessamPublishedScope, and don't take effect if the logged in user can manageb log posts
        ...
        if ($category_slug) {
            ...
            $posts = $category->posts()->where("hessam_post_categories.hessam_category_id", $category->id);
            ...
            \View::share('hessam_category', $category); // so the view can say "You are viewing $CATEGORYNAME category posts"
            $title = 'Posts in ' . $category->category_name . " category"; // hardcode title here...
        } else {
            $posts = HessamPost::query();
        }

        $posts = $posts->where('is_published', '=', 1)->where('posted_at', '<', Carbon::now()->format('Y-m-d H:i:s'))->orderBy("posted_at", "desc")->paginate(config("hessam.per_page", 10));
    ...
    {
        // the published_at + is_published are handled by HessamPublishedScope, and don't take effect if the logged in user can manage log posts
        ...
        return view("hessam::single_post", [
            'post' => $blog_post,
