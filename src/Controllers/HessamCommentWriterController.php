...
use Auth;
use HessamDev\Hessam\Captcha\CaptchaAbstract;
use HessamDev\Hessam\Captcha\UsesCaptcha;
use HessamDev\Hessam\Events\CommentAdded;
use HessamDev\Hessam\Models\HessamComment;
use HessamDev\Hessam\Models\HessamPost;
use HessamDev\Hessam\Requests\AddNewCommentRequest;
...
class HessamCommentWriterController extends Controller
{
    ...
    public function addNewComment(AddNewCommentRequest $request, $blog_post_slug)
    {
        ...
        if (config("hessam.comments.type_of_comments_to_show", "built_in") !== 'built_in') {
        ...
        return view("hessam::saved_comment", [
            'captcha' => $captcha,
            'blog_post' => $blog_post,
    ...
    protected function createNewComment(AddNewCommentRequest $request, $blog_post)
    {
        ...
        if (config("hessam.comments.save_ip_address")) {
            $new_comment->ip = $request->ip();
        }
        if (config("hessam.comments.ask_for_author_website")) {
            $new_comment->author_website = $request->get('author_website');
        }
        if (config("hessam.comments.ask_for_author_website")) {
            $new_comment->author_email = $request->get('author_email');
        }
        if (config("hessam.comments.save_user_id_if_logged_in", true) && Auth::check()) {
            $new_comment->user_id = Auth::user()->id;
        }

        $new_comment->approved = config("hessam.comments.auto_approve_comments", true) ? true : false;
