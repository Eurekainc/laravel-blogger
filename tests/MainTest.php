...
//    use \Illuminate\Foundation\Testing\DatabaseTransactions;
    ...
    /blog/...
        hessam.index                       YES
        hessam.feed                        YES
        hessam.view_category               no - but this is basically hessam.index
        hessam.single                      YES
        hessam.comments.add_new_comment    YES - tested multiple times with/without basic captcha on/off/correct/incorrect.
    ...
    /blog_admin/...
        hessam.admin.index                 YES
        hessam.admin.create_post           no - but is just a form
        hessam.admin.store_post            YES
        hessam.admin.edit_post             YES - but no extra checks
        hessam.admin.update_post           YES
        hessam.admin.destroy_post          YES

     /blog_admin/comments/...

            hessam.admin.comments.index    YES
            hessam.admin.comments.approve  YES
            hessam.admin.comments.delete   YES

     /blog_admin/categories/...

            hessam.admin.categories.index
            hessam.admin.categories.create_category    no - but is just a form
            hessam.admin.categories.store_category     YES
            hessam.admin.categories.edit_category      no - but is just a form
            hessam.admin.categories.update_category
            hessam.admin.categories.destroy_category   YES
    ...
    {
        $this->assertFileExists(config_path("hessam.php"), "/config/hessam.php should exist - currently no file with that filename is found");
        $this->assertTrue(is_array(include(config_path("hessam.php"))), "/config/hessam.php should exist - currently no file with that filename is found");
    }

    public function testImageSizesAreSane()
    {

        $this->assertTrue(count(config("hessam.image_sizes")) >=  3);

        foreach (config("hessam.image_sizes") as $image_key => $image_info) {
    ...
    public function testUserHasNanManageHessamPostsMethod()
    {

        $this->assertTrue(method_exists(\App\User::class, "canManageHessamPosts"), "Your User model must have the canManageHessamPosts method");

        $user = new \App\User();
        $this->assertTrue(is_bool($user->canManageHessamPosts()));
    ...
    public function testCanSeeAdminPanel()
    {
        ...
        $admin_panel_url = config("hessam.admin_prefix", "blog_admin");
...
//        $user = new \App\User();
        ...
        $this->assertTrue($user->canManageHessamPosts());
...
//        $new_object_vals['use_view_file'] = "";
        ...
        $justCreatedRow = \HessamDev\Hessam\Models\HessamPost::where("slug", $new_object_vals['slug'])->firstOrFail();
...
//        $new_object_vals['use_view_file'] = "";
        ...
        $response = $this->get(config("hessam.blog_prefix", "blog"));
        $response->assertDontSee($new_object_vals['slug']);

        // must clear the cache, as the /feed is cached
        \Artisan::call('cache:clear');

        $response = $this->get(config("hessam.blog_prefix", "blog") . "/feed");
        $response->assertDontSee($new_object_vals['slug']);
        ...
        $response = $this->get(route("hessam.single", $new_object_vals['slug']));
        $response->assertStatus(200);
        $response->assertSee($new_object_vals['slug']);
        $response->assertSee($new_object_vals['title']);
    ...
    public function testCreatePostWithNotPublishedThenCheckIsNotViewableToPublic()
    {
        ...
        $response = $this->get(config("hessam.blog_prefix", "blog") . "/" . $new_object_vals['slug']);
    ...
    public function testCreatePostThenCheckCanCreateCommentThenApproveCommentWithBasicCaptchaEnabledAndWrongAnswer()
    {
        ...
        \Config::set('hessam.comments.auto_approve_comments', false);
        \Config::set('hessam.captcha.captcha_enabled', true);
        \Config::set('hessam.captcha.captcha_type', \HessamDev\Hessam\Captcha\Basic::class);
        $captcha = new \HessamDev\Hessam\Captcha\Basic();
        \Config::set('hessam.captcha.basic_question', "a test question");
        \Config::set('hessam.captcha.basic_answers', "answer1,answer2");
...
//        $new_object_vals['use_view_file'] = "";
        ...
        \Config::set('hessam.comments.type_of_comments_to_show', 'built_in');
        ...
        $response = $this->post(config("hessam.blog_prefix", "blog") . "/save_comment/" . $new_object_vals['slug'], $comment_detail);
        $response->assertStatus(302);
    ...
    public function testCreatePostThenSetCommentsToDisabledAndCheckNoneShow()
    {
        ...
        \Config::set('hessam.comments.type_of_comments_to_show', "disabled");
        ...
        $newblogpost = new \HessamDev\Hessam\Models\HessamPost;
    ...
    {
        \Config::set('hessam.comments.type_of_comments_to_show', "disqus");
    ...
    public function testCreatePostThenSetCommentsToCustomAndCheckCustomErrorShows()
    {
        ...
        \Config::set('hessam.comments.type_of_comments_to_show', "custom");
...
//        $new_object_vals['use_view_file'] = "";
        ...
        \Config::set('hessam.captcha.auto_approve_comments', false);

        $this->assertDatabaseHas('blog_etc_comments', ['approved' => false, 'author_name' => $comment_detail['author_name']]);


        $justAddedRow = \HessamDev\Hessam\Models\HessamComment::withoutGlobalScopes()->where('author_name', $comment_detail['author_name'])->firstOrFail();

        $response = $this->get(route("hessam.admin.comments.index"));
        $response->assertSee($justAddedRow->author_name);


        // approve it:
        $response = $this->patch(route("hessam.admin.comments.approve", $justAddedRow->id), [
            '_token' => csrf_token(),
        ]);
    ...
    {
        \Config::set('hessam.comments.auto_approve_comments', false);
        \Config::set('hessam.captcha.captcha_enabled', false);
...
//        $new_object_vals['use_view_file'] = "";
        ...
        if (config("hessam.comments.type_of_comments_to_show") === 'built_in') {
            $comment_detail = [
                '_token' => csrf_token(),
                'author_name' => str_random(),
                'comment' => str_random(),
            ...
            // delete it:
            $response = $this->delete(route("hessam.admin.comments.delete", $justAddedRow->id), [
                '_token' => csrf_token(),
            ]);
            ...
            dump("NOT TESTING COMMENT FEATURE, as config(\"hessam.comments.type_of_comments_to_show\") is not set to 'built_in')");
        }
...
//        $new_object_vals['use_view_file'] = "";
    ...
    public function testCanCreateCategoryThenEditIt()
    {
        ...
        $justCreatedRow = \HessamDev\Hessam\Models\HessamCategory::where("slug", $new_cat_vals['slug'])->firstOrFail();
        ...
        $response = $this->patch(
            route("hessam.admin.categories.update_category", $justCreatedRow->id),
            $new_object_vals
        );
    ...
    protected function create_admin_user()
    {
        ...
        $user->method("canManageHessamPosts")
...
//        $new_object_vals['use_view_file'] = "";
    ...
    public function testUserModelHasCanManageHessamPostsMethod()
    {
        $u = new \App\User();
        $this->assertTrue(method_exists($u,"canManageHessamPosts"),"canManageHessamPosts() must be added to User model. Please see HessamDev Hessam docs for details. It should return true ONLY for the admin users");
    }
    public function testUserModelCanManageHessamPostsMethodDoesntAlwaysReturnTrue()
    {
        $u = new \App\User();

        $u->id = 9999999; // in case the logic on canManageHessamPosts() checks for a low ID
        $u->email = str_random(); // in case the logic looks for a certain email or something.

        $this->assertTrue(method_exists($u,"canManageHessamPosts"));

        // because this user is just a randomly made one, it probably should not be allowed to edit blog posts.
        $this->assertFalse($u->canManageHessamPosts(), "User::canManageHessamPosts() returns true, but it PROBABLY should return false! Otherwise every single user on your site has access to the blog admin panel! This might not be an error though, if your system doesn't allow public registration. But you should look into this. I know this isn't a good way to handle this test, but it seems to make sense.");
    }
